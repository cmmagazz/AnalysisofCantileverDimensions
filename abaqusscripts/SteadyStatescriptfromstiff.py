...
#Creating a for loop to iterate through all frames in the step: in stiffness there's only 1!
frameno=1

#NEW METHOD - STRESS
#NEWER METHOD BUT SLOW - getSubset
frame=odb.steps['stiffness'].frames[frameno]
#start with stress
...
for y in range(firstel,len(odb.rootAssembly.instances['PART-1-1'].nodeSets['EDGE'].nodes)):
    ...
    deformedcoordinates[y-firstel]=odb.steps['stiffness'].frames[frameno].fieldOutputs['U'].values[nodeslabelforcorners[y-firstel]-1].data[::2]
    changeincoordinates =deformedcoordinates-originalcoordinates
positionof4corners=np.reshape(changeincoordinates, numel*2)
temp=np.insert(positionof4corners,[0, 0],[[frameno,avestress]])
