...
function [SSstressregression,erroracrossfreq]=runinputSTEADYSTATE(testdims,n,maxparralel,filepath)
% currdate=clock;
% currdate=currdate(1:3);
currdate=[];
outputname=['steadystatepostcalib7-11_PC3' num2str(currdate)];

amplitudes=[5,10];% how many amplitudes are tested and which ones
...
for i = 1:n %set up the job names
    for j=1:size(amplitudes,2) 
        ...
        jobname{pos} = ['SS33ir_for', num2str(floor(testdims(i,1))),'_',num2str(floor(testdims(i,2))),'_',...
            num2str(floor(testdims(i,3))),'_',num2str(floor(testdims(i,4))),'_',...
            num2str(floor(testdims(i,5))),'_',num2str(floor(testdims(i,6))),'_',num2str(amplitudes(j))];
    end
end
...
for i = 1:n %set up the results file
    for j=1:size(amplitudes,2) 
        for k=1:frequencies
            pos=4*i-4+2*j-2+k;
            maxstress(pos,1:8)=[testdims(i,1),testdims(i,2),testdims(i,3),testdims(i,4),...
                testdims(i,5),testdims(i,6),amplitudes(j),0];
        end
    end
end
...
for i=1:n*size(amplitudes,2) %loop through all tex files that were created files
    ...
    try
        ...
        C=cell2mat(C);
        try
            maxstress(2*i-1:2*i,8:end)=C(2:end,:);
        catch
            try
                maxstress(2*i-1,8:end)=C(2:end,:);
...
Xcoords=maxstress(:,10:2:end);
Zcoords=maxstress(:,11:2:end);
%figure();
...
SSstress=[maxstress(:,1:9) angle];

%% now calculate the stress/angle

%Step 1 is average across frequencies:
SSstressavgfreq=zeros(size(maxstress,1)/2,10);
SSstress=abs(SSstress);
for i =1:size(SSstress,1)/2
    SSstressavgfreq(i,1:7)=SSstress(2*i-1,1:7); %get the dims
    SSstressavgfreq(i,8:10)=mean(SSstress(2*i-1:2*i,8:10)); %average across frequency, stress, and angle
    stdevstressacrossfreq(i,:)=std(SSstress(2*i-1:2*i,8:10)); %USEFUL FOR DEBUG
end
